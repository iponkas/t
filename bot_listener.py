"""
Слушает Telegram: обрабатывает /menu, /stop, /resume, нажатия кнопок и
ввод даты, обновляет config.json.

Не заходит на сайт mostanet.ru — этим занимается отдельно check_tickets.py
(по часовому расписанию). Этот скрипт лёгкий и запускается часто
(каждые 1-2 минуты), чтобы бот отвечал быстро.

Запускается через GitHub Actions, см. .github/workflows/bot-listener.yml
"""
...
def process_updates(config: dict) -> dict:
