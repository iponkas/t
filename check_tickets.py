...
def send_telegram_message(text: str) -> None:
    ...
    try:
        ...
        # Печатаем настоящую причину отказа от Telegram, чтобы было видно в логе
        error_body = e.read().decode("utf-8", errors="replace")
        print(f"Telegram API вернул ошибку: {error_body}")
        raise
...
def check_tickets() -> bool:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # небольшая дополнительная пауза — на случай, если результат
            # дорисовывается уже после networkidle
            page.wait_for_timeout(1500)

            content = page.content()
        finally:
            # Сохраняем скриншот момента проверки — пригодится для диагностики,
            # даже если что-то пошло не так на одном из шагов выше
